...
factory_production_menu = { _this spawn {
	...
	buttonSetAction [factory_enqueue_button_id, format['if (ctrlText factory_amount_field_id < 0) then {[ctrlText factory_amount_field_id] call factory_dupe_log;}; [%1, "%2", (lbData [factory_enqueue_list_id, (lbCurSel factory_enqueue_list_id)]), ([(ctrlText factory_amount_field_id)] call parse_number)] call factory_item_enqueue;', _player, _factory_id]];
